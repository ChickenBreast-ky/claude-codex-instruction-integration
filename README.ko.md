...
clone 한 저장소 루트에서 설치 스크립트를 실행합니다.

```bash
./scripts/install.sh
```

기존 동일 경로가 이미 있고, 그것을 `~/.trash-staging/` 으로 먼저 옮긴 뒤 교체하고 싶다면:
...
- `scripts/install.sh`
  - 스킬을 `~/.claude/skills` 로 복사하고 `~/.codex/skills` 에 링크를 만드는 설치 스크립트입니다.
...
- 설치 스크립트를 `--force` 로 실행하면 충돌하는 기존 경로를 `~/.trash-staging/` 으로 먼저 옮긴 뒤 교체합니다.
